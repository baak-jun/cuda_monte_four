...
constexpr std::uint32_t kMagic = 0x31424443u; // "CDB1", little-endian.
...
class PerfectDb {
public:
    explicit PerfectDb(const Options& options) : options_(options) {
    ...
    void load(const std::string& path) {
        ...
        if (header.magic != kMagic || header.version != kVersion || header.record_size != sizeof(FileRecord)) {
            throw std::runtime_error("unsupported DB format: " + path);
        }
        if (header.rows != connect4::kRows || header.cols != connect4::kCols || header.stride != connect4::kStride) {
            throw std::runtime_error("DB board layout does not match this build");
        }

        table_.reserve(static_cast<std::size_t>(std::min<std::uint64_t>(header.record_count, options_.reserve)));
    ...
    Result solve(const connect4::Board& board, int depth) {
        ...
        if (options_.limit_states != 0 && table_.size() >= options_.limit_states) {
        ...
        bool has_win = false;
        std::int8_t win_move = -1;
        std::int8_t draw_move = -1;
        std::int8_t unknown_move = -1;
        std::int8_t first_loss_move = -1;
        ...
        for (int col : move_order()) {
            ...
            if (child == Result::Loss) {
                has_win = true;
                if (win_move < 0) {
                    win_move = static_cast<std::int8_t>(col);
                }
            ...
            } else if (first_loss_move < 0) {
                first_loss_move = static_cast<std::int8_t>(col);
            }
        ...
        if (has_win) {
            ...
            entry.best_move = first_loss_move;
        }
    ...
    void save(const std::string& path) const {
        ...
        if (!out) {
            throw std::runtime_error("failed to save DB: " + path);
        }
        ...
        const FileHeader header{
            ...
            static_cast<std::uint8_t>(connect4::kStride),
            {0, 0, 0, 0, 0},
        };
    ...
    void save_text(const std::string& path) const {
        ...
        if (!out) {
            throw std::runtime_error("failed to save CSV: " + path);
        }
...
private:
    ...
    static const std::vector<int>& move_order() {
    ...
    static void write_pod(std::ofstream& out, const T& value) {
        ...
        if (!out) {
            throw std::runtime_error("failed to write DB");
        }
    }
...
Options parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--moves") {
            ...
            std::cout
                << "Usage: perfect_db_cpu [--load DB] [--save DB] [--moves COLUMNS]\n"
                << "                      [--max-depth N] [--limit-states N] [--text CSV]\n"
                << "\n"
                << "Builds or queries a persistent exact minimax DB. Results are for the side to move.\n";
            std::exit(0);
        } else {
...
int main(int argc, char** argv) {
    try {
        ...
        PerfectDb db(options);
        if (options.load_path.has_value()) {
            db.load(*options.load_path);
        }

        const connect4::Board board = board_from_moves(options.moves);
        const Result result = db.solve(board, static_cast<int>(board.moves));
        const std::optional<Entry> entry = db.lookup(board);

        if (options.save_path.has_value()) {
            db.save(*options.save_path);
        }
        if (options.text_path.has_value()) {
            db.save_text(*options.text_path);
        }

        const Stats& stats = db.stats();
        std::cout << "result " << result_name(result) << '\n';
        std::cout << "best_move " << (entry.has_value() ? static_cast<int>(entry->best_move) : -1) << '\n';
        std::cout << "states " << db.size() << '\n';
        ...
        if (options.max_depth < connect4::kMaxMoves) {
            std::cout << "note max-depth is partial DB, not a full proof\n";
        }
